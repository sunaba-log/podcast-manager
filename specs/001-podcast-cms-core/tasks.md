...
- [ ] T001 `/frontend` と `/backend` ディレクトリ構造を作成
- [ ] T002 [P] Next.js 14+ (App Router) フロントエンドプロジェクト初期化
- [ ] T003 [P] Express/Node.js バックエンドプロジェクト初期化
- [ ] T004 [P] TypeScript 5.x および必須パッケージをセットアップ（Prisma, Zod, Shadcn UI）
- [ ] T005 [P] ESLint/Prettier 設定ファイルを作成（`.eslintrc.json`, `.prettierrc`）
- [ ] T006 [P] Git pre-commit hook を設定（lint 自動実行）
- [ ] T007 PostgreSQL ローカル開発環境を `docker-compose.yml` に設定
...
- [ ] T008 Prisma スキーマを `/backend/prisma/schema.prisma` に定義（User, Podcast, Episode, AudioFile, Artwork, TeamMember エンティティ）
- [ ] T009 [P] Prisma マイグレーションスクリプトを `/backend/prisma/migrations/` に作成
- [ ] T010 [P] Prisma ORM クライアントを `/backend/src/lib/db.ts` に実装
- [ ] T011 [P] JWT 認証ミドルウェアを `/backend/src/api/middleware/auth.ts` に実装
- [ ] T012 [P] 認可・ロール管理ロジックを `/backend/src/lib/authorization.ts` に実装
- [ ] T013 [P] Zod バリデーションスキーマを `/backend/src/lib/validators.ts` に定義
- [ ] T014 [P] API エラーハンドリング統一フォーマットを `/backend/src/lib/errors.ts` に実装
- [ ] T015 [P] ロギング設定を `/backend/src/lib/logger.ts` に実装
- [ ] T016 Google Cloud Storage (GCS) SDK 統合を `/backend/src/lib/gcs.ts` に実装（署名付きURL生成、削除イベント）
- [ ] T017 [P] 環境変数設定テンプレートを `.env.example` に作成
- [ ] T018 [P] Next.js API ルーティング構造を `/frontend/src/app/api/` に作成
- [ ] T019 [P] フロントエンド Shadcn UI コンポーネント基盤を `/frontend/src/components/ui/` に作成
- [ ] T020 [P] フロントエンド API クライアント実装を `/frontend/src/lib/api.ts` に作成
...
- [ ] T021 [P] [US1] Podcast モデルと migration を `/backend/prisma/schema.prisma` に定義
- [ ] T022 [P] [US1] 番組作成エンドポイント `POST /api/shows` を `/backend/src/api/routes/shows.ts` に実装
- [ ] T023 [P] [US1] 番組取得エンドポイント `GET /api/shows/:id` を `/backend/src/api/routes/shows.ts` に実装
- [ ] T024 [P] [US1] 番組一覧エンドポイント `GET /api/shows` を `/backend/src/api/routes/shows.ts` に実装
- [ ] T025 [P] [US1] 番組更新エンドポイント `PUT /api/shows/:id` を `/backend/src/api/routes/shows.ts` に実装
- [ ] T026 [US1] Podcast ビジネスロジックサービスを `/backend/src/services/podcast.service.ts` に実装（依存：T022-T025）
...
- [ ] T031 [US1] 番組メタデータ入力検証を `/backend/src/lib/validators.ts` に追加（依存：T013）
...
- [ ] T034 [P] [US2] Artwork モデルとリレーションを `/backend/prisma/schema.prisma` に追加
- [ ] T035 [P] [US2] 画像検証ユーティリティを `/backend/src/lib/image-validator.ts` に実装（サイズ・形式チェック）
- [ ] T036 [US2] 画像アップロード受け付けエンドポイント `POST /api/shows/:showId/artwork` を `/backend/src/api/routes/artwork.ts` に実装（依存：T035）
- [ ] T037 [US2] 画像取得エンドポイント `GET /api/shows/:showId/artwork` を `/backend/src/api/routes/artwork.ts` に実装
- [ ] T038 [US2] 画像削除エンドポイント `DELETE /api/shows/:showId/artwork` を `/backend/src/api/routes/artwork.ts` に実装
...
- [ ] T044 [P] [US3] Episode モデルを `/backend/prisma/schema.prisma` に定義（Podcast との関連付け）
- [ ] T045 [P] [US3] エピソード作成エンドポイント `POST /api/shows/:showId/episodes` を `/backend/src/api/routes/episodes.ts` に実装
- [ ] T046 [P] [US3] エピソード取得エンドポイント `GET /api/shows/:showId/episodes/:episodeId` を `/backend/src/api/routes/episodes.ts` に実装
- [ ] T047 [P] [US3] エピソード一覧エンドポイント `GET /api/shows/:showId/episodes` を `/backend/src/api/routes/episodes.ts` に実装
- [ ] T048 [P] [US3] エピソード更新エンドポイント `PUT /api/shows/:showId/episodes/:episodeId` を `/backend/src/api/routes/episodes.ts` に実装
- [ ] T049 [US3] Episode ビジネスロジックサービスを `/backend/src/services/episode.service.ts` に実装（依存：T045-T048）
...
- [ ] T054 [US3] エピソード入力値検証スキーマを `/backend/src/lib/validators.ts` に追加（依存：T013）
...
- [ ] T056 [P] [US4] AudioFile モデルを `/backend/prisma/schema.prisma` に定義（Episode との関連付け）
- [ ] T057 [P] [US4] GCS 署名付きURL生成エンドポイント `POST /api/audio/signed-url` を `/backend/src/api/routes/audio.ts` に実装
- [ ] T058 [P] [US4] 音声ファイルメタデータ登録エンドポイント `POST /api/shows/:showId/episodes/:episodeId/audio` を `/backend/src/api/routes/audio.ts` に実装
- [ ] T059 [P] [US4] 音声ファイル取得エンドポイント `GET /api/shows/:showId/episodes/:episodeId/audio` を `/backend/src/api/routes/audio.ts` に実装
- [ ] T060 [US4] エピソード削除時の削除イベント送信を `/backend/src/services/episode.service.ts` に実装（依存：T049）
...
- [ ] T065 [P] [US4] 音声ファイル形式検証を `/backend/src/lib/validators.ts` に追加
...
- [ ] T067 [P] [US5] RSS 生成ロジックを `/backend/src/services/feed.service.ts` に実装（Podcast Namespace 対応）
- [ ] T068 [P] [US5] Enclosure URL に R2 公開 URL を含めるロジックを追加（`feed.service.ts` 内）
- [ ] T069 [P] [US5] RSS フィード取得エンドポイント `GET /feeds/shows/:showId/rss.xml` を `/backend/src/api/routes/feeds.ts` に実装
- [ ] T070 [P] [US5] XML 検証ユーティリティを `/backend/src/lib/xml-validator.ts` に実装
- [ ] T071 [US5] RSS キャッシング戦略を `/backend/src/lib/cache.ts` に実装（パフォーマンス最適化）
...
- [ ] T075 [P] [US6] Podcast に `feedUrl` フィールドを `/backend/prisma/schema.prisma` に追加
- [ ] T076 [P] [US6] 番組作成時に固定 RSS URL を生成するロジックを `/backend/src/services/podcast.service.ts` に追加（依存：T026）
- [ ] T077 [US6] フィード URL 取得エンドポイント `GET /api/shows/:showId/feed-url` を `/backend/src/api/routes/feeds.ts` に実装
...
- [ ] T081 [P] [US7] User-Podcast 関連付けロジックを Prisma スキーマ内で実装（1対多）
- [ ] T082 [P] [US7] ユーザーの番組一覧エンドポイント `GET /api/shows?userId=...` を修正（フィルタリング追加）
- [ ] T083 [P] [US7] 現在のユーザーコンテキスト取得エンドポイント `GET /api/me/current-show` を `/backend/src/api/routes/auth.ts` に実装
...
- [ ] T088 [P] [US8] TeamMember モデルをスキーマに追加（ロール: admin, editor）
- [ ] T089 [P] [US8] ロールベースアクセス制御（RBAC）を `/backend/src/lib/authorization.ts` に実装（依存：T012）
- [ ] T090 [P] [US8] チームメンバー招待エンドポイント `POST /api/shows/:showId/members/invite` を `/backend/src/api/routes/team.ts` に実装
- [ ] T091 [P] [US8] チームメンバー一覧エンドポイント `GET /api/shows/:showId/members` を `/backend/src/api/routes/team.ts` に実装
- [ ] T092 [P] [US8] チームメンバー削除エンドポイント `DELETE /api/shows/:showId/members/:userId` を `/backend/src/api/routes/team.ts` に実装
- [ ] T093 [P] [US8] ロール変更エンドポイント `PUT /api/shows/:showId/members/:userId/role` を `/backend/src/api/routes/team.ts` に実装
- [ ] T094 [P] [US8] メール招待メッセージテンプレートを `/backend/src/templates/invite-email.html` に作成
- [ ] T095 [P] [US8] メール送信ロジックを `/backend/src/services/email.service.ts` に実装
- [ ] T096 [P] [US8] 招待受け入れエンドポイント `POST /api/invitations/:token/accept` を `/backend/src/api/routes/auth.ts` に実装
